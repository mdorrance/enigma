...
class Cracking
  ...
  def total_rotate_a
    @offset_generator.offset_a + @key_generator.chars.values_at(0,1).join.to_i
  end

  # The second character is rotated forward by the "B" rotation plus the "B offset"

  def total_rotate_b
    @offset_generator.offset_b + @key_generator.chars.values_at(1,2).join.to_i
  end

  # The third character is rotated forward by the "C" rotation plus the "C offset"
  def total_rotate_c
    @offset_generator.offset_c + @key_generator.chars.values_at(2,3).join.to_i
  end
